...
CREATE TABLE users (
	...
	created_by	BIGINT		CONSTRAINT fk_users_created_by
...
COMMENT ON TABLE users IS
	'Logins.  Never deleted -- every send_by and created_by depends on these rows.';

COMMENT ON COLUMN users.created_by IS
	'Nullable only for the bootstrap account, which has no creator.';
