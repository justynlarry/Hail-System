...
CREATE TABLE email_templates (
	...
	supersedes_id	BIGINT
			CONSTRAINT fk_email_templates_supersedes
...
COMMENT ON COLUMN email_templates.report_type IS
	...
	'report_type_pair_complete keeps it a pair: MATCH SIMPLE skips the composite '
	'FK entirely when either column is null.';
...
CREATE TABLE send_log (
	...
	realtor_id	BIGINT	NOT NULL
			CONSTRAINT fk_send_log_realtor
			REFERENCES realtors (realtor_id),
	recipient_email	TEXT	NOT NULL,

	match_id	BIGINT	NOT NULL
			CONSTRAINT fk_send_log_match
			REFERENCES storm_listing_matches (match_id),
	template_id	BIGINT	NOT NULL
			CONSTRAINT fk_send_log_template
			REFERENCES email_templates (template_id),

	queued_at	TIMESTAMPTZ	NOT NULL DEFAULT now(),
	sent_at		TIMESTAMPTZ,
	sent_by		BIGINT		NOT NULL
			CONSTRAINT fk_send_log_sent_by
			REFERENCES users (emp_id),
	...
	CONSTRAINT sent_has_timestamp
		CHECK (send_status = 'queued' OR send_status = 'failed'
			OR sent_at IS NOT NULL)
);
 
CREATE INDEX send_log_realtor_id_sent_at_idx ON send_log (realtor_id, sent_at);
CREATE INDEX send_log_match_id_idx	   ON send_log (match_id);

COMMENT ON TABLE send_log IS
	'Append-Only: it is the audit trail, and can show if/when a suppressed email '
...
COMMENT ON COLUMN send_log.provider_message_id IS
	'Opaque id from the ESP.  Named _id because it holds an identifier, not message '
	'content -- it is how a bounce or complaint notification finds its way back to '
