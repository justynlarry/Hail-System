...
CREATE TABLE api_pulls (
	...
	estimated_api_calls	INTEGER		NOT NULL CHECK (estimated_calls >= 0),
	actual_api_calls	INTEGER		CHECK (actual_calls > 0),
...
CREATE INDEX api_pulls_emp_started_idx ON api_pulls (emp_id, started_at DESC);

COMMENT ON TABLE api_pulls IS
	'This records what the system spend in API calls.'

COMMENT ON COLUMN api_pulls.iem_id IS
	'Nullable - a pull does not need to be tied to a specific storm.'

COMMENT ON COLUMN api_pulls.estimated_calls IS
...
CREATE TABLE api_call_log (
	...
	pull_id		BIGINT		NOT NULL REFERENCES api_pulls (pull_id),
...
CREATE INDEX api_call_log_zip_idx ON api_call_log (zip_code, called_at DESC);
CREATE INDEX api_call_log_pull_idx ON api_call_log (pull_id);
